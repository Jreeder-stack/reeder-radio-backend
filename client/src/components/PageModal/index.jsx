...
export default function PageModal({ target, onClose }) {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(d.error || 'Failed to send page');
      }
      const data = await res.json();
  ...
  return (
    ...
    >
      <div
        style={{
          background: 'var(--dispatch-bg, #1a1a2e)',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', color: 'var(--dispatch-secondary, #888)', cursor: 'pointer', fontSize: 20, lineHeight: 1 }}
          >
            ✕
          </button>
        </div>
        ...
        {!sentPage ? (
          <form onSubmit={handleSend}>
            <div style={{ marginBottom: 8, fontSize: 13, color: 'var(--dispatch-secondary, #888)' }}>
              Target: <strong style={{ color: 'var(--dispatch-text, #eee)' }}>{target.label}</strong>
              {' '}({target.type === 'unit' ? 'Single Unit' : target.type === 'channel' ? 'Channel' : 'All Units'})
            </div>
            <textarea
              value={message}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button
                ...
                style={{
                  ...
                  color: 'var(--dispatch-secondary, #888)',
                  cursor: 'pointer',
                  fontSize: 13,
                }}
              >
          ...
          <div>
            ...
            <h4 style={{ margin: '0 0 10px', fontSize: 13, fontWeight: 600, color: 'var(--dispatch-text, #eee)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Delivery Confirmation
            </h4>

            {targetedUnits.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--dispatch-secondary, #888)' }}>
                No FCM-registered devices targeted. Units must have the app running with notifications enabled.
              </p>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 7,
                        background: 'var(--dispatch-surface, #222)',
                        border: `1px solid ${acked ? 'rgba(34,197,94,0.4)' : 'var(--dispatch-border, #333)'}`,
                      }}
                    >
                      <span style={{ fontSize: 16 }}>{acked ? '✅' : '⏳'}</span>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, color: 'var(--dispatch-text, #eee)', fontWeight: 500 }}>
                          {unit.unitId || unit.radioId}
                        </div>
                        {acked && ackRecord && (
                          <div style={{ fontSize: 11, color: '#86efac', marginTop: 2 }}>
                            Received {new Date(ackRecord.acked_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                          </div>
                        )}
                        {!acked && (
                          <div style={{ fontSize: 11, color: 'var(--dispatch-secondary, #888)', marginTop: 2 }}>Pending...</div>
                        )}
                      </div>
                    </div>
                  );
