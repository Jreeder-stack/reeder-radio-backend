...
- **Inrico T320 Support (240x320 screen):** When screen width <=280px, or screen.width <=280px, or Capacitor native + screen <=480px, a dedicated display-only layout renders - a Motorola APX-style white-background screen with top status bar (clock/signal/battery/scan/lock), centered zone name (bold), large channel name, and "ID: {unitId}" for TX/RX caller. Bottom dark soft-key menu bar shows SCNL (scan status), clock, battery %. TX shows red "TX" indicator; RX shows green with caller ID; emergency shows full-screen red flash. No on-screen buttons; all controls are mapped to T320 physical hardware keys via JavaScript keydown/keyup listeners: PTT (keycode 230) = transmit, DPAD Up/Down (19/20) = channel up/down, DPAD Left/Right (21/22) = zone down/up, Emergency (233) = long-press 2s to activate / 3s to deactivate (with visual hold progress bar), ACC (231) = scan toggle, Star (17) long-press 1s = key lock toggle. Volume Up/Down (24/25) left unmapped for native volume control. Key lock: long-press Star for 1 second to lock all keys (except volume); long-press again to unlock. Shows blinking "LCK" indicator in status bar. When scanning and receiving, display switches to show the talking channel's zone/channel/ID. MicPTTManager handles TX state transitions. The `HardwarePttPlugin.java` defaults to keycode 230 for T320 compatibility. **Key Event Injection:** `MainActivity.java` intercepts all T320 hardware keys, wakes the screen if off (via PowerManager + KeyguardManager), and injects synthetic JavaScript `KeyboardEvent` into the WebView via `evaluateJavascript()`, since Android 7.0 WebView doesn't reliably forward hardware key events to JavaScript.
- **Auto-Login:** Credentials are saved in localStorage after successful login. On app restart, if the session cookie has expired, the app automatically re-authenticates using saved credentials and skips the login screen. Logout clears saved credentials. Works on both mobile (T320/phone) and desktop.
