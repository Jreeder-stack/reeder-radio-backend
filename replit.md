...
- **Radio Client (Siyata SD7):** Same Kotlin codebase, built as a separate APK via the `sd7` Gradle product flavor (`applicationIdSuffix = ".sd7"`). Hardware integration differs from the T320: the SD7 firmware exposes its physical buttons as broadcast intents (Airbus PMR / Kodiak / `android.intent.action.*` namespaces), confirmed via on-device logcat capture. `KEYCODE_F11` (141) is treated as PTT on the T320 build only — on the SD7 build that exact keycode is the SOS button, gated by `BuildConfig.FLAVOR`. SD7 side-button volume keys use a 600 ms hold-timer in `MainActivity` to distinguish short-press (system volume) from long-press (top = scan toggle, bottom = toggle current channel on/off scan list). The SD7 OLED (`Sd7RadioStatusScreen`) renders `ZN:<zone>` / `CH:<channel>` on the top two lines, with a `✓` next to the channel name when it is on the scan list.
  ...
  **SD7 OLED & headless UX (Task #579 research, corrected by Task #586):** The SD7's 0.97" 128×64 OLED is owned and painted by the firmware app `com.br.smallcd` (Belfone "smallcd"). **Correction (2026-05-13):** the OLED **is** writable from our own third-party APK — `com.br.smallcd` registers a public Android framework system service `android.app.SmallcdManager` (service name `smallcd`), and any app can `context.getSystemService("smallcd")` then call `init(128, 64, "/dev/oled_display")`, `getAppId(pkg)`, `drawText/drawBitmap/fillRect/refresh`, plus `reverseDisplay`/`covertMode` for emergency/stealth. Full API contract (parameter-by-parameter, with real call-site evidence from the decompiled firmware), the `_ExtDisplayMsg`/`SMALLCD_EXT` external-message path, the 65-class `_Display` library, system hooks (NFC Wi-Fi onboarding, FOTA, LocationAccessibilityService, covert mode), the permissions story for our SD7 APK, recommended 128×64 layouts, and the prioritized follow-up task slate live in `android-native/SD7_FIRMWARE_INVENTORY.md`. The existing audio/haptic/TTS UX in `android-native/SD7_HEADLESS_UX.md` §4 stays valid as the cue layer; it is now complemented (not replaced) by the OLED writer. The `Sd7RadioStatusScreen` Compose UI is retained as the **engineer-only debug surface** (visible in emulator / `adb screencap`); the operator-facing surface on real SD7 hardware will be the OLED renderer landed by the F1 follow-up task.
