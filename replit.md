...
- **AI Voice Dispatcher:** Integrates Azure Speech Services for STT/TTS and Azure OpenAI (GPT-4o-mini) for natural language intent classification. The LLM replaces rigid regex pattern matching — units can speak naturally (e.g., "throw me on duty", "put me on a detail at Wawa") and the AI understands the intent. Falls back to regex matching if Azure OpenAI is not configured. Emergency commands (10-33, officer down, shots fired) still go through fast pattern matching first for zero-latency safety. The dispatcher operates in standby mode, connecting to LiveKit on demand, with per-unit conversation sessions and state-aware behavior. It supports status changes, zone changes, detail commands, 10-27 records checks with phonetic spelling, traffic stops, plate checks, backup requests, radio/time checks, Signal 100, emergency escalation, and **voice-driven CAD call creation**. CAD integration for automatic status updates and queries. TTS voice is configurable via `AI_DISPATCHER_VOICE` env var (default: `en-US-GuyNeural`). A 1.5-second audio idle timeout safeguards PTT release reliability. Late-join track pickup ensures the dispatcher catches audio even when joining after a unit starts transmitting. Track SID deduplication prevents double-processing.
- **10-27 Records Check Flow:** Unit says "Central, 10-27" → AI responds "[unit], 10-27 go ahead" → Unit provides name/DOB → AI asks for any missing fields (last name, first name, DOB) individually → AI reads back details and asks "10-4?" → Unit confirms (10-4) or denies (negative to re-enter) → AI queries CAD → Results returned (with secure mic check for flagged individuals). DOB parsing supports numeric (3/15/1985), spoken month ("march fifteenth nineteen eighty five"), and fully spoken ("three fifteen eighty five", "oh three fifteen eighty five") formats. Name parsing handles inline spelling confirmation: dash-separated letters ("Smith S-M-I-T-H"), dotted letters ("R.E.E.D.E.R"), phonetic alphabet ("Romeo Echo Echo Delta Echo Romeo"), and "spelled" prefix.
- **Detail Command Flow:** Unit says "Central, show me out on a detail at [location]" → AI confirms "detail at [location], 10-4?" → Unit confirms → CAD updates BOTH status to "detail" AND zone to the location. Supports inline location extraction (skips intermediate prompt) or prompted location entry. Uses `AWAITING_DETAIL_LOCATION` and `AWAITING_DETAIL_CONFIRM` states.
- **Voice-Driven CAD Call Creation Flow:** Unit says "Central, show me out on a [nature] at [address]" (or "start me a call for [nature] at [address]") → AI fuzzy-matches the spoken nature against the live CAD call natures list (fetched from `GET /api/radio/call-natures`, cached 30 min) → AI confirms "[unit], confirm, [nature] at [address]?" → Unit confirms → `cadService.createCall()` + `cadService.assignUnitToCall()` + auto-sets unit to `en_route`. Supports inline extraction (nature + address in one phrase), multi-step prompted entry (`AWAITING_CALL_NATURE` → `AWAITING_CALL_ADDRESS` → `AWAITING_CALL_CONFIRM`), and additional unit assignment. LLM intents: `CREATE_CALL` (with slots), `CREATE_CALL_PROMPT` (missing info). Disregard/cancel works at any step.
- **Zone Change Command Flow:** Unit says "Central, change my zone to [address]" (or "show me out at [address]", "relocating to [address]") → AI confirms zone → Unit confirms/denies → CAD `updateUnitZone()` called. Uses `AWAITING_ZONE` and `AWAITING_ZONE_CONFIRM` states. Detail command patterns are checked before zone patterns to prevent "show me out on a detail at X" from matching the zone-only "show me out at X" pattern.
...
- **Azure OpenAI (GPT-4o-mini):** Used by the AI Voice Dispatcher for natural language intent classification. Configured via `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_DEPLOYMENT` env vars. Falls back to regex if not configured.
