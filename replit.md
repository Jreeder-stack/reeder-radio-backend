...
- **Inrico T320 Support (240x320 screen):** When screen width <=280px, or screen.width <=280px, or Capacitor native + screen <=480px, a dedicated display-only layout renders - a white-background screen showing unit ID, labeled ZONE and CHANNEL sections, TX/RX indicator with caller ID, and emergency flash. No on-screen buttons; all controls are mapped to T320 physical hardware keys via JavaScript keydown/keyup listeners: PTT (keycode 230) = transmit, DPAD Up/Down (19/20) = channel up/down, DPAD Left/Right (21/22) = zone down/up, Emergency (233) = emergency toggle, ACC (231) = scan toggle, Star (17) long-press = key lock toggle. Volume Up/Down (24/25) left unmapped for native volume control. Key lock: long-press Star for 1 second to lock all keys (except volume); long-press again to unlock. Shows blinking "LOCKED" indicator in header. MicPTTManager handles TX state transitions. The `HardwarePttPlugin.java` defaults to keycode 230 for T320 compatibility. **Key Event Injection:** `MainActivity.java` intercepts all T320 hardware keys and injects synthetic JavaScript `KeyboardEvent` into the WebView via `evaluateJavascript()`, since Android 7.0 WebView doesn't reliably forward hardware key events to JavaScript.
