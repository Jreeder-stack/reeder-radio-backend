...
- **Radio Device Auth (T320):** Hardware radios (T320) use device-identity-based auth instead of username/password. On first install, the app registers via `POST /api/radios/register` (serial + IMEI), receives a permanent Radio ID and token stored in `RadioTokenStore` (SharedPreferences). Navigation bypasses LoginScreen when a radio token is present: no token → DeviceRegistrationScreen; token present → UnassignedScreen (waits for `radio:assigned` Socket.IO event); assigned → main RadioScreen. `radio:locked` shows LockedScreen; `radio:unlocked` recovers automatically. Socket.IO connects with `radioToken` query param. Device identity (`DeviceIdentity`) reads serial via `Build.getSerial()` and IMEI via `TelephonyManager`, with graceful null fallback on permission denial. Auto-backup is disabled (`android:allowBackup="false"` + `data_extraction_rules.xml`) to prevent stale tokens from being restored on fresh installs. On startup, `AppNavigation` validates stored tokens against the server (`/api/radios/ping`) and clears invalid/stale credentials, forcing re-registration. A guard also catches the case where a token exists but radioId is missing/blank.
...
- **Priority Channel:** In the Dispatch Console, dispatchers can designate one channel as "priority" via a star button on each ChannelTile. Only the priority channel's audio is played; all other monitored channels have their audio suppressed but still show RX activity badges. Priority selection persists across reloads. On Android (T320), the currently selected channel is automatically the priority channel during scan mode — only its audio is received and only its RX activity is displayed.
