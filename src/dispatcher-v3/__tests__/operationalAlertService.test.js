...
describe('V3OperationalAlertService', () => {
  it('activates emergency state only on the selected channel and suppresses global fan-out', () => {
    ...
    const result = service.declareEmergency({
      identity,
      runtimeContext: runtime,
      correlationId: 'corr-emerg',
      callId: 'call-1',
      location: '100 Main St',
      reason: 'officer needs assistance',
    });

    expect(result.activated).toBe(true);
    expect(result.delivery).toEqual({ channelScoped: true, globalPushSuppressed: true });
    expect(signaling.emergencyStates.get('OPS1')).toMatchObject({
      unitId: 'INDIANA-1',
      unitUuid: 'uuid-1',
      dispatchCenterId: 'center-1',
      correlationId: 'corr-emerg',
  ...
  it('rejects a second unit trying to own the same channel emergency state', () => {
    ...
    expect(() => service.declareEmergency({
      identity,
      runtimeContext: runtime,
      correlationId: 'corr-2',
    })).toThrow(expect.objectContaining({ code: 'CAD_REJECTED', statusCode: 409 }));
  });

  it('sends backup only to the selected channel with center metadata', () => {
    const signaling = makeSignaling();
    ...
    expect(signaling._emitToChannelDispatchers).toHaveBeenCalledWith('OPS1', 'backup:request', result.backup);
  ...
  it('fails closed if signaling is not initialized', () => {
    ...
    expect(() => service.requestBackup({
      identity,
      runtimeContext: runtime,
      correlationId: 'corr-3',
    })).toThrow(expect.objectContaining({ code: 'CAD_UNAVAILABLE' }));
  });
});
